...
// 자리배치도의 좌석 외 배치 요소 (Seating 도메인 §2.8, Phase 9) — 테이블/구역 네모/
// 출입문/텍스트. 좌석(venue_seats)과 달리 순수 표시용이라 참여자와 연결되지 않는다.
// 조회는 공개(손님 방 화면도 같은 배치를 그려야 함), 쓰기는 매장 소유자만.

const VALID_KINDS: LayoutItemKind[] = ['table', 'box', 'door', 'text']
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  const { id } = await params
  const { kind, label, position_x, position_y, width, height } = await req.json()
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      width: typeof width === 'number' ? clampPct(width, 3, 100) : size.width,
      height: typeof height === 'number' ? clampPct(height, 3, 100) : size.height,
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  const { id } = await params
  const { item_id, label, position_x, position_y, width, height } = await req.json()
  ...
  if (typeof width === 'number') payload.width = clampPct(width, 3, 100)
  if (typeof height === 'number') payload.height = clampPct(height, 3, 100)
