...
export async function POST(req: NextRequest, { params }: { params: Promise<{ code: string }> }) {
  ...
  const { participant_id, message } = await req.json()

  if (!participant_id || typeof message !== 'string' || !message.trim()) {
    return NextResponse.json({ error: '손님, 메시지가 필요합니다' }, { status: 400 })
  }
