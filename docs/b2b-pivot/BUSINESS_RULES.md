...
**정의**: 위 모든 도메인의 데이터를 읽기 전용으로 재가공해 대시보드/퍼널/리포트를 제공한다. `operation_events`(운영자가 직접 기록하는 이벤트 타임라인, Staff 도메인과 함께 Phase 6에서 다룸)를 제외하면 자체 쓰기 상태를 갖지 않는다.

| 규칙 | 상태 | 내용 |
|------|------|------|
| 원본 데이터 미수정 | 확정 | 대시보드/퍼널/리포트는 Reaction/Chat/Participant 등 다른 도메인의 데이터를 조회만 할 뿐, 어떤 값도 직접 쓰거나 수정하지 않는다 |
| 이벤트 기록 예외 | 확정 (Phase 6 예정) | `operation_events`(직원 교대 외 운영자가 직접 기록하는 이벤트: 게임 시작, 음악 변경, 서비스 제공 등)만 이 도메인이 직접 쓰기를 소유한다 |
