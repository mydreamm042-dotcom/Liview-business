...
| **Seating** (좌석/자리배치) 🆕 | 매장 내부의 물리적 좌석 배치를 운영자가 구성하고, 참여자가 입장 시 좌석을 선택하며, 좌석 단위로 실시간 HOT 활동을 보여주는 도메인. **운영자가 좌석을 클릭해 수행하는 관리자 조작(경고/서비스/쿠폰/강제 좌석이동 등)은 아직 스펙 미확정** | `venue_seat_layouts`(신설 예정), `participants.seat_id`(신설 예정) | Venue, Room, Participant, Reaction |
...
- **Seating 도메인의 운영자 조작 기능은 아직 정의되지 않았다**: "경고 메시지/서비스/쿠폰/관리자 권한 좌석 이동" 등은 사용자가 아직 상세를 설명하지 않았으므로, 이 문서는 그 규칙을 추측해서 만들지 않는다. 설명을 받는 즉시 이 섹션과 로드맵을 갱신한다 (`CHANGE_INTAKE_PROCESS.md` 참고).
...
### 2.7 Seating 도메인 (좌석/자리배치) — 일부만 확정, 진행 중

**정의**: 매장 내부의 물리적 좌석 배치를 운영자가 구성하고, 참여자가 BUSINESS 방 입장 시 좌석을 선택하며, 좌석 단위로 실시간 HOT 활동을 보여준다.

> ⚠️ **이 도메인은 절반만 스펙이 확정됐다.** 아래 표의 "확정" 항목은 사용자가 설명한 내용을 그대로 옮긴 것이고, "미확정" 항목은 아직 설명을 듣지 못해 규칙을 만들지 않은 것이다. 임의로 채워 넣지 않는다.
...
| 좌석별 HOT 표시 | 확정 | 자리배치도에는 각 좌석에 앉은 참여자가 "지금 HOT을 탭하고 있는지"가 표시된다. 이 값은 `reactions`(hot)를 참여자의 좌석 배정과 조인해 계산하는 파생값이며, 좌석 전용 원본 데이터를 새로 만들지 않는다 (Atmosphere Index 도메인과 동일 원칙) |
| 좌석 선택 필수 여부 | **미확정** | 좌석을 반드시 선택해야 방에 참여할 수 있는지, 스킵하고 익명으로만 참여할 수도 있는지 확인 필요 |
| 운영자 좌석 관리 기능 (경고 메시지/서비스/쿠폰/관리자 권한 좌석 이동) | **미확정 — 스펙 대기 중** | 사용자가 "아직 설명하지 않았다"고 명시함. 설명을 받는 즉시 이 표와 로드맵을 갱신한다 |

### 2.8 Operator Analytics 도메인
...
### 2.9 참여자 방 나가기(Exit Flow) — 도메인 간 순서 규칙
