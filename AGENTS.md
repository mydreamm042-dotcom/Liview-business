...
- **브랜치 이름 규칙**: `feature/<설명>`(새 기능) · `fix/<설명>`(버그 수정) · `chore/<설명>`(설정/버전/도구 등 기능 아닌 변경) · `docs/<설명>`(문서 전용인데 예외적으로 브랜치가 필요한 경우) — 케밥 케이스, 영문. **설명은 의도로 짓는다, 확정 안 된 버전 번호를 박아 넣지 않는다** (예: `chore/v0.3.0-release` ❌ → `chore/release-bookkeeping` ✅ — 버전은 리뷰 중 바뀔 수 있어서 이름과 어긋날 위험이 있다)
